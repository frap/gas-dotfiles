...
#+AUTHOR: Andrés Gasson straight copy from Howard Abrahms
...
** Emacs Executable
   ...
   While Howard often uses [[http://emacsformacosx.com/builds][Emacs for Mac]], lately, I've been building Emacs
...
** Init File Support
   ...
   and [[https://github.com/magnars/s.el][s]] for string manipulation, as much of my initialisation code
   depends on it.
...
** Better Jumping
   ...
   The [[https://github.com/tam17aki/ace-isearch][ace-isearch]] looks like a simple approach to integrate many
   types of searches into a single entry point:
   ...
   #+BEGIN_SRC elisp
     ...
     specified.  Select the current line if the LINES prefix is zero."
         ...
         (cond ((= lines 1)   (er/expand-region 1))
               ((< lines 0)   (gas/expand-previous-line-as-region lines))
               (t             (gas/expand-next-line-as-region (1+ lines)))))

       (defun gas/expand-next-line-as-region (lines)
         (message "lines = %d" lines)
         (beginning-of-line)
         (set-mark (point))
         (end-of-line lines))

       (defun gas/expand-previous-line-as-region (lines)
         (end-of-line)
         (set-mark (point))
         (beginning-of-line (1+ lines)))

       :bind ("C-=" . gas/expand-region))
   #+END_SRC
   ...
   #+BEGIN_SRC elisp  tangle:no
     (use-package bookmark
       ...
       (defun gas/add-bookmark (name)
         ...
         (bookmark-set name))
       :bind  (("C-c b m" . gas/add-bookmark)
               ("C-x r m" . gas/add-bookmark)
